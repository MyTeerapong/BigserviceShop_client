...
useEffect(() => {
  if (details.length > 0 && product.length > 0) {
  ...
  const handleDetailChange = async (index, field, value) => {
    ...
    if (selectedProduct) {
        ...
        newDetails[index].P_quantity = selectedProduct.P_quantity || 0; 
    } else {
...
const handleSubmit = async (e) => {
  ...
  try {
    await axios.put(`http://localhost:3000/api/sale/update/${S_id}`, payload , getconfig());
